...
${BIRDWEATHER_ID_LOG}
    ...
    if [ -n "${SPECTROGRAM_FILTER}" ]; then
      sox -V1 "${1}/${i}" -n ${SPECTROGRAM_FILTER} remix 1 rate "${SAMPLING_RATE}" spectrogram -o "${spectrogram_png}"
    else
      sox -V1 "${1}/${i}" -n remix 1 rate "${SAMPLING_RATE}" spectrogram -o "${spectrogram_png}"
    fi
