...
${BIRDWEATHER_ID_LOG}
    ...
    if [ -n "${SPECTROGRAM_FILTER}" ]; then
      sox -V1 "${1}/${i}" -n ${SPECTROGRAM_FILTER} remix 1 spectrogram -x "${SAMPLING_RATE}" -c "${analyzing_now//$HOME\//}" -o "${spectrogram_png}"
    else
      sox -V1 "${1}/${i}" -n remix 1 spectrogram -x "${SAMPLING_RATE}" -c "${analyzing_now//$HOME\//}" -o "${spectrogram_png}"
    fi
