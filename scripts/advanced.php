...
if(isset($_GET['submit'])) {
  ...
  if (isset($_GET["activate_freqshift_in_livestream"])) {
    ...
    if (strcmp($activate_freqshift_in_livestream, $config['ACTIVATE_FREQSHIFT_IN_LIVESTREAM']) !== 0) {
      if (strpos($contents, "ACTIVATE_FREQSHIFT_IN_LIVESTREAM=") !== false) {
        ...
        $contents .= "ACTIVATE_FREQSHIFT_IN_LIVESTREAM=\"$activate_freqshift_in_livestream\"\n";
        $contents2 .= "ACTIVATE_FREQSHIFT_IN_LIVESTREAM=\"$activate_freqshift_in_livestream\"\n";
      }
      save_to_cfg($contents, $contents2);
      exec("sudo systemctl restart livestream.service");
    }
...
?>
    ...
    <form action="" method="GET">
      ...
      <input name="recording_length" oninput="document.getElementsByName('extraction_length')[0].setAttribute('max', this.value);" type="number" min="1" max="28" step="1" value="<?php print($newconfig['RECORDING_LENGTH']);?>" required/><br>
      <p>Set Recording Length in seconds between 1 and 28. Multiples of 1 are recommended.</p>
      <label for="extraction_length">Extraction Length: </label>
      <input name="extraction_length" oninput="this.setAttribute('max', document.getElementsByName('recording_length')[0].value);" type="number" value="<?php print($newconfig['EXTRACTION_LENGTH']);?>" /><br>
      <p>Set Extraction Length to something less than your Recording Length. Min=0.2 Max=Recording Length</p>
